...
type 'a grammar = {
  ...
  mutable parse : 'b. grouped -> buffer -> int -> next option -> (buffer -> int -> buffer -> int -> 'a -> 'b) -> 'b PosMap.t ;
}
...
let eof : 'a -> 'a grammar
  = fun a ->
	...
	let str, pos = apply_blank grouped str pos in
	  if is_empty str then single str pos (g str pos str pos a) else parse_error grouped (~~ "EOF") str pos
    }
...
let sequence : 'a grammar -> 'b grammar -> ('a -> 'b -> 'c) -> 'c grammar
  ...
  let flag = ref false in
    { firsts = lazy (union_firsts l1 l2);
      ...
      parse =
	fun grouped str pos next g ->
	  let la = l1.parse grouped str pos (union' l2 next) (fun _ _ _ _ x -> x) in
	  let res = PosMap.fold (fun (str',pos') a acc ->
	    try
	      let res = merge_map1 str pos acc (l2.parse grouped str' pos' next
...
let merge_sequence : ('c -> 'c -> 'c) -> 'a grammar -> 'b grammar -> ('a -> 'b -> 'c) -> 'c grammar
  ...
  let flag = ref false in
	...
	fun grouped str pos next g ->
	  ...
	  let res = PosMap.fold (fun (str', pos') a acc ->
	    try	      
	      let res = merge_map2 merge acc (l2.parse grouped str' pos' next (fun _ _ l' pos' x -> str, pos, l', pos', f a x)) in
	      res
	    with Give_up -> acc
	  ) la PosMap.empty in
...
let dependent_sequence : 'a grammar -> ('a -> 'b grammar) -> 'b grammar
  ...
  let flag = ref false in
	...
	false);
      parse =
	fun grouped str pos next g ->
	  let la = l1.parse grouped str pos None (fun  _ _ _ _ x -> x) in
	  let res = PosMap.fold (fun (str', pos') a acc ->
	    try
	      let res = merge_map1 str pos acc ((f2 a).parse grouped str' pos' next (fun _ _ l' pos' -> g str pos l' pos')) in
	      res
	    with Give_up -> acc
	  ) la PosMap.empty in
	  if res = PosMap.empty then raise Give_up else res
...
let dependent_merge_sequence : ('b -> 'b -> 'b) -> 'a grammar -> ('a -> 'b grammar) -> 'b grammar
  ...
  let flag = ref false in
	...
	fun grouped str pos next g ->
	  ...
	  let res = PosMap.fold (fun (str', pos') a acc ->
	    try
	      let res = merge_map2 merge acc ((f2 a).parse grouped str' pos' next (fun _ _ l' pos' x -> str, pos, l', pos', x)) in
	      res
	    with Give_up -> acc
	  ) la PosMap.empty in
	  if res = PosMap.empty then raise Give_up else PosMap.map (fun (l, pos, l', pos', x) -> g l pos l' pos' x) res
...
let change_layout : ?old_blank_before:bool -> ?new_blank_after:bool -> 'a grammar -> blank -> 'a grammar
  = fun ?(old_blank_before=true) ?(new_blank_after=false) l1 blank1 ->
	...
	fun grouped str pos next g  ->
	  ...
	  let res = l1.parse { grouped with blank = blank1 } str pos None g in
...
let option' : 'a -> 'a grammar -> 'a grammar
  = fun a l ->
    { firsts = lazy (firsts l);
      ...
      parse =
	fun grouped str pos next g ->
	  let acc = if test grouped next str pos then single str pos (g str pos str pos a) else PosMap.empty in
	  try
	    let res = merge_map1 str pos acc (l.parse grouped str pos next g) in
	    res
	  with
	    Give_up ->
...
let merge_option : ('a -> 'a -> 'a) -> 'a -> 'a grammar -> 'a grammar
  = fun merge a l ->
    ...
    { firsts = lazy (firsts l);
      ...
      parse =
	fun grouped str pos next g ->
	  let acc = if test grouped next str pos then single str pos (str, pos, str, pos, a) else PosMap.empty in
	  try
	    let res = merge_map2 merge acc (l.parse grouped str pos next (fun l pos l' pos' x -> l, pos, l', pos', x)) in
...
let option : 'a -> 'a grammar -> 'a grammar
  = fun a l ->
	...
	fun grouped str pos next g ->
	  try
	    let res = l.parse grouped str pos next g in
	    res
	  with
	    Give_up ->
	      if test grouped next str pos then single str pos (g str pos str pos a) else raise Give_up;
    }
...
let fixpoint' : 'a -> ('a -> 'a) grammar -> 'a grammar
  = fun a f1 ->
	...
	fun grouped str pos next g ->
	  ...
	  let rec fn acc la =
	    PosMap.fold (fun (str', pos') a acc ->
	      let acc = if test grouped next str' pos' then merge_map1 str pos (single str' pos' (g str pos str' pos' a)) acc else acc in
	      (try
		 let r = f1.parse grouped str' pos' next' (fun _ _ _ _ f -> f a) in
		 fun () -> fn acc r
	       with
...
let merge_fixpoint : ('a -> 'a -> 'a) -> 'a -> ('a -> 'a) grammar -> 'a grammar
  = fun merge a f1 ->
	...
	fun grouped str pos next g ->
	  ...
	  let rec fn acc la =
	    PosMap.fold (fun (str', pos') a acc ->
	      let acc = if test grouped next str' pos' then merge_map2 merge (single str' pos' (str, pos, str', pos', a)) acc else acc in
	      (try
		 let r = f1.parse grouped str' pos' next' (fun _ _ _ _ f -> f a) in
		 fun () -> fn acc r
	       with
	  ...
	  let rec fn acc la =
	    PosMap.fold (fun (str', pos') a acc ->
		 ...
		 fun () -> fn acc r
	       with 
		 Give_up ->
		 if test grouped next str' pos' then (fun () -> single str' pos' (g str pos str' pos' a)) else fun () -> acc) ()) la acc
	  in
	  let res = fn PosMap.empty (single str pos a) in
	  if res = PosMap.empty then raise Give_up;
	  res
	...
	fun grouped str pos next g ->
	  ...
	  let rec fn acc = function
	    ...
	    | l::ls ->
	      let acc = 
		try
		  let acc = merge_map1 str pos acc (l.parse grouped str pos next g) in
		  acc
		with
		  Give_up -> acc
	      in fn acc ls
	...
	fun grouped str pos next g ->
	  ...
	  let rec fn acc = function
	    ...
	    | l::ls ->
	      let acc = 
		try
		  merge_map2 merge acc (l.parse grouped str pos next (fun l pos l' pos' x -> l, pos, l', pos', x))
		with
		  Give_up -> acc
	      in fn acc ls
	  in
...
let parse_buffer grammar blank str =
  ...
  let la = try
      grammar.parse grouped str 0 None (fun _ _ _ _ x -> x) 
    with Give_up -> 
...
let partial_parse_buffer grammar blank str pos = 
  ...
  let grouped = { blank; err_key = key; ignore_next_blank = str, -1 } in
  let la = try
      grammar.parse grouped str pos None (fun _ _ _ _ x -> x)
    with Give_up -> 
      let str, pos, msgs = try Hashtbl.find max_hash key with Not_found -> str, 0, Empty in
      raise (Parse_error (fname str, line_num str, pos, collect_tree msgs))
  in
