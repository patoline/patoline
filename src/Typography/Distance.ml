...
let profile_union (dsup, dinf) c1 c2 =
  ...
  let cons p l = match l with
      [] -> [p]
    | p'::_ -> if near p p' then l else p::l
  in
...
(*   Printf.fprintf stderr "\nStart\n"; *)
  ...
  let rec fn acc c1 c2 = 
    ...
    | p1::c1', p2::c2' ->
      ...
      if x1 <= x2 then
	if dot_prod p2 dinf' >= dot_prod p1 dinf' then
	  fn acc c1' c2
	else if dot_prod p1 dsup' >= dot_prod p2 dsup' then
	  fn acc c1 c2'
	else
	  fn (cons p1 acc) c1' c2
      else
	if dot_prod p1 dinf' >= dot_prod p2 dinf' then
	  fn acc c1 c2'
	else if dot_prod p2 dsup' >= dot_prod p1 dsup' then
	  fn acc c1' c2
	else
	  fn (cons p2 acc) c1 c2'

      | [], _::_ -> List.fold_left (fun acc x -> cons x acc) c2 acc
      | _, [] -> List.fold_left (fun acc x -> cons x acc) c1 acc
...
let eqdist d p o i1 i2 =
  ...
  let s, n = 
	...
	Printf.fprintf stderr "  n1 = %a n2 = %a \n  s1 = %f s2 = %f\n"
      ...
      let x1 = dot_prod (n1 -- i2) (i1 -- i2) /. norm2 (i1 -- i2) in
      let x2 = dot_prod (n2 -- i2) (i1 -- i2) /. norm2 (i1 -- i2) in

      match 
	in_range o s1 && x1 > -1e-7 && x1 < 1. +. 1e-7,
	in_range o s2 && x2 > -1e-7 && x2 < 1. +. 1e-7
      with
	true, false -> s1, n1
      | false, true -> s2, n2
...
let mediatrice_profile (dsup,dinf) profile1 profile2 =
  ...
  let cons (x,i,y as t) = function
      [] -> [t]
    | (x',i',y')::l' as l ->
      if near x x' && near i i' && near y y' then t :: l' else t :: l
  in
  ...
  let cons2 p1 q1 i p2 q2 acc =
    if near p1 p2 && near q1 q2 then
      ...
      (p1, i, p2)::(x, i, y)::(cons (q1, i, q2) acc)
  in

  let rec push x = function
      [] -> [x]
    | [x'] when near x x' -> [x']
    | x'::(x''::_ as l) when abs_float (det22 (x' -- x'') (x -- x'')) < 1e-6 -> push x l
    | l -> x::l
  in
  ...
  let rec fn acc state c1 c2 =
	...
	fn (cons (p1,i,p2) acc) (LeftParabola(p1,next dsup p1 c1',cut' p2 o2)) c1' c2

      |  [], (p2::c2') -> 

	let i = try inter b (Line(p2,w)) with Not_found -> assert false in
	let p1 = try inter o1 (Line(i,v)) with Not_found -> assert false in
	fn (cons (p1,i,p2) acc) (RightParabola(cut' p1 o1, next dsup' p2 c2', p2)) c1 c2'
	...
	if abs_float x < 1e-8 then
	  let i = try inter b (Line(p2,w)) with Not_found -> assert false in
	  fn (cons (p1,i,p2) acc) (Mediatrice(p1, next dsup p1 c1', next dsup' p2 c2', p2)) c1' c2'
	else if x > 0.0 then
	  let i = try inter b (Line(p1,v)) with Not_found -> assert false in
	  let n2 = try inter o2 (Line(i,w)) with Not_found -> assert false in
	  fn (cons (p1,i,n2) acc) (LeftParabola(p1,next dsup p1 c1',cut' n2 o2)) c1' c2
	else
	  let i = try inter b (Line(p2,w)) with Not_found -> assert false in
	  let n1 = try inter o1 (Line(i,v)) with Not_found -> assert false in
	  fn (cons (n1,i,p2) acc) (RightParabola(cut' n1 o1, next dsup' p2 c2', p2)) c1 c2')
	 ...
	 fn (cons (p1,i,n2) acc) (Bissectrice(o1,cut' n2 o2)) c1 c2
       with
       | Not_found -> fn acc (LeftBack(p1,o1,o2)) c1 c2
       | Extrem -> 
	   match c2 with [] -> assert false | p2::c2' ->
	     let i = find_parabola p1 o2 p2 in
	     fn (cons (p1,i,p2) acc) (Mediatrice(p1,o1,next dsup' p2 c2',p2)) c1 c2')
	 ...
	 fn (cons (n1,i,p2) acc) (Bissectrice(cut' n1 o1,o2)) c1 c2
       with
       | Not_found -> fn acc (RightBack(o1,o2,p2)) c1 c2
       | Extrem -> 
	   match c1 with [] -> assert false | p1::c1' ->
	     let i = find_parabola p2 o1 p1 in
	     fn (cons (p1,i,p2) acc) (Mediatrice(p1,next dsup p1 c1', o2, p2)) c1' c2)
     ...
     | Mediatrice(p1,o1,o2,p2) ->
	   ...
	   (r1,i,r2)::(q1,_,q2)::_ -> (q1 -- r1),i , (q2 -- r2)
	 | [_,i,_] -> dinf,i,dinf'
	 | _ -> assert false
       in
       let b1 = abs_float (det22 u1 v1) < 1e-6 in
       let b2 = abs_float (det22 u2 v2) < 1e-6 in
	 ...
	 try let i1 = try inter m (Line(p1,v1')) with Not_found -> assert false in
	   if b1' && b2' &&  dot_prod (i1 -- i) v > -1e-7 then
	     fn (cons (p1,i1,p2) acc) (Bissectrice(o1,o2)) c1 c2
	   else raise Not_found
	 with Not_found ->
	   if x > 0.0 then 
	 ...
	 try let i1 = inter m (Line(p1,v1')) in
	     ...
	     if b1' && (b1 ||  dot_prod (i1 -- i) v > -1e-7) then
	       fn (cons (p1,i1,p2) acc) (RightParabola(o1,o2,p2)) c1 c2
	     else raise Not_found
	 with Not_found -> 
	   fn acc (LeftBack(p1,o1,o2)) c1 c2
       else
	 try let i2 = inter m (Line(p2,v2')) in
	     ...
	     if b2' && (b2 || dot_prod (i2 -- i) v > -1e-7) then 
	       fn (cons (p1,i2,p2) acc) (LeftParabola(p1,o1,o2)) c1 c2
	     else raise Not_found
	 with Not_found -> 
	     fn acc (RightBack(o1,o2,p2)) c1 c2)
	 ...
	 try 
	   ...
	   if near r1 q1 then
		 ...
		 let i, on1 = eqdist m r2 o1 i1 i2 in
		 ...
		 let i, on1 = eqdist b r1 o1 i1 i2 in
		 ...
		 let i, n2 = eqdist m oq1 (Segment(r2,q2)) i1 i2 in
		 ...
		 let i, on1 = eqdist b r2 o1 i1 i2 in
		 ...
		 let i, n1 = eqdist m oq1 (Segment(r1,q1)) i1 i2 in
		 ...
		 let i, n2 = try eqdist m oq1 (Segment(r2,q2)) i1 i2 with Not_found -> raise Not_found in
		 let n1 = project i (Segment(r1,q1)) in
	 ...
	 try 
	   ...
	   if near r2 q2 then
	     if near r1 q1 then
	       ...
	       try
		 if !debug_mediatrice then Printf.fprintf stderr "TRY Mediatrice inter\n";
		 let i, on2 = eqdist m r1 o2 i1 i2 in 
		 if !debug_mediatrice then Printf.fprintf stderr "Mediatrice inter\n";
		 ...
		 if !debug_mediatrice then Printf.fprintf stderr "TRY Parabola opp inter\n";
		 let i, on2 = eqdist b r2 o2 i1 i2 in
		 let n1 = project i (Segment(r1,q1)) in
		 if !debug_mediatrice then Printf.fprintf stderr "Parabola opp inter\n";
  		 let o2', c2', p2' = forward on2 oq2 c2 c2' in
		 ...
		 let i, n1 = try eqdist m oq2 (Segment(r1,q1)) i1 i2 with Not_found -> raise Not_found in
		 if !debug_mediatrice then Printf.fprintf stderr "Parabola opp extremity\n";
		 let o1', c1' = back o1 r1 n1 c1 in
	     ...
	     if near r1 q1 then
	       try
		 if !debug_mediatrice then Printf.fprintf stderr "TRY Parabola inter\n";
		 let i, on2 = eqdist b r1 o2 i1 i2 in
		 let n2 = project i (Segment(r2,q2)) in
		 if !debug_mediatrice then Printf.fprintf stderr "Parabola inter\n";
  		 let o2', c2', p2' = forward on2 oq2 c2 c2' in
		 ...
		 let i, n2 = try eqdist m oq2 (Segment(r2,q2)) i1 i2 with Not_found -> raise Not_found in
		 if !debug_mediatrice then Printf.fprintf stderr "Parabola extremity\n";
		 ...
		 if !debug_mediatrice then Printf.fprintf stderr "TRY Bissectrice inter b = %a\n" print_obj b;
		 let i = inter b m in
		 if !debug_mediatrice then Printf.fprintf stderr "TRY(2) Bissectrice interi = %a\n" print_pt i;
