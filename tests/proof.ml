    ...
    let rec make_line i maxi x y0 y1 l=
      ...
      let l,y0,y1=make_line i0 (i0+10) 0. infinity (-.infinity) [] in
        if l=[] then [{ pageFormat=a4;
                        pageContents=p}] else (
          if y -. (y1-.y0)*.size/.1000. >= bot then (
            let finaly= y -. y1*.size/.1000. in
            make_pages (i0+10) (finaly +. y0*.size/.1000.-.lead ) ((Path ({Drivers.default with lineWidth=0.1}, [ [|[|0.;210.|],[|finaly;finaly|]|] ]))::
                                                              (* (Path ({Drivers.default with lineWidth=0.1}, [|[|0.;210.|],[|y;y|]|])):: *)
                                                              (List.map (translate left_margin (finaly)) l) @ p)
          ) else
            ({ pageFormat=a4;
               pageContents=p})::(make_pages i0 270. [])
        )
  in
    Drivers.Pdf.output (Array.of_list ((make_pages 0 270. []))) ((Filename.chop_extension f)^".pdf")
